...
class RangeSlider extends StatefulWidget {
  ...
  ///     log('Started change with $startLowerValue and $startUpperValue');
  ///   },
  /// )
  /// ```
  ///
  ...
  ///     log('Ended change with $newLowerValue and $newUpperValue');
  ///   },
  /// )
  /// ```
  ///
...
class _RenderRangeSlider extends RenderBox {
  ...
  double computeMinIntrinsicWidth(double height) {
    return 2 * math.max(_overlayDiameter, _sliderTheme.thumbShape.getPreferredSize(true, (_divisions != null)).width);
  }
  ...
  double computeMinIntrinsicHeight(double width) {
    return math.max(_overlayDiameter, _sliderTheme.thumbShape.getPreferredSize(true, (_divisions != null)).height);
  }
  ...
  void _paintTrack(Canvas canvas, Offset offset) {
    ...
    Paint unselectedTrackPaint = Paint()..color = isInteractive ? _sliderTheme.inactiveTrackColor : _sliderTheme.disabledInactiveTrackColor;
    Paint selectedTrackPaint = Paint()..color = isInteractive ? _sliderTheme.activeTrackColor : _sliderTheme.disabledActiveTrackColor;

    // Draw the track
    if (_lowerValue > 0.0) {
      // Draw the unselected left range
      canvas.drawRect(Rect.fromLTRB(_trackLeft, _trackTop, _thumbLeftPosition, _trackBottom), unselectedTrackPaint);
    }
    // Draw the selected range
    canvas.drawRect(Rect.fromLTRB(_thumbLeftPosition, _trackTop, _thumbRightPosition, _trackBottom), selectedTrackPaint);

    if (_upperValue < 1.0) {
      // Draw the unselected right range
      canvas.drawRect(Rect.fromLTRB(_thumbRightPosition, _trackTop, _trackRight, _trackBottom), unselectedTrackPaint);
    }
  }
  ...
  void _paintTickMarks(Canvas canvas, Offset offset) {
    ...
    for (int i = 0; i <= _divisions; i++) {
      ...
      canvas.drawCircle(center, _tickRadius, Paint()..color = isInteractive ? _sliderTheme.activeTickMarkColor : _sliderTheme.disabledActiveTickMarkColor);
    }
  }
  ...
  void _paintThumbs(PaintingContext context, Offset offset) {
    final Offset thumbLowerCenter = Offset(_thumbLeftPosition, _trackVerticalCenter);
    final Offset thumbUpperCenter = Offset(_thumbRightPosition, _trackVerticalCenter);
    final double thumbRadius = _thumbRadius;

    _thumbLowerRect = Rect.fromCircle(center: thumbLowerCenter - offset, radius: thumbRadius);
    _thumbUpperRect = Rect.fromCircle(center: thumbUpperCenter - offset, radius: thumbRadius);
  ...
  _validateActiveThumb(Offset position) {
    var _thumbLowerExpandedRect = Rect.fromCircle(center: _thumbLowerRect.centerLeft, radius: _thumbRadius * _touchRadiusExpansionRatio);
    var _thumbUpperExpandedRect = Rect.fromCircle(center: _thumbUpperRect.centerRight, radius: _thumbRadius * _touchRadiusExpansionRatio);
    double calculatedDivisionOffset = (_divisions != null) ? _discretize(1.0 / _divisions) : (_thumbRadius * 2.0) / _trackLength;
